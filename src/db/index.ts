...
abstract class Database {
	...
	static initDb({ host, port, options, main }: DBOptions) {
		...
		try {
			...
			t.start("connect");
			Logger.debug("Database", `Connecting to ${dbString} (SSL: ${options.ssl ? "Yes" : "No"})`);
			// eslint-disable-next-line @typescript-eslint/unbound-method
			this.connection = deasync(MongoClient.connect)(dbString, options) as MongoClient;
			t.end("connect");
			Logger.debug("Database", `Connected to ${dbString} (SSL: ${options.ssl ? "Yes" : "No"}) in ${t.calc("connect")}ms`);
		} catch (e) {
			// eslint-disable-next-line @typescript-eslint/restrict-template-expressions
			Logger.error("Database", `Error connecting to MongoDB instance (${dbString}, SSL: ${options.ssl ? "Yes" : "No"})\nReason: ${"stack" in e ? (e as { stack: string; }).stack : e}`);
	...
	static initRedis({ host, port, password, db, name }: RedisOptions) {
		...
		r
			.on("connect", () => Logger.debug("Redis", `Connected to redis://${host}:${port} (db: ${db})`));
	}
